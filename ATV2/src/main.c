...
static const char fw_version[] = {'V', '1', '.', '1'};
...
/* Configura PC0 como saida pull-up ON e pull-down OFF */
reg = *pGPIOC_MODER;
reg &= ~(GPIO_MODER0_MASK);
reg |= (GPIO_MODER_OUTPUT << GPIO_MODER0_SHIFT);
*pGPIOC_MODER = reg;
reg = *pGPIOC_OTYPER;
reg &= ~(GPIO_OT0_MASK);
...
if((*pGPIOC_BSRR || GPIO_BSRR_SET(0)) == true){

LED_TIME_MS = 400;}

else LED_TIME_MS = 100;

*pGPIOC_BSRR = GPIO_BSRR_SET(13);
for(uint32_t i = 0; i < LED_TIME_MS; i++);
